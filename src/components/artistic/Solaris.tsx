...
export default function Solaris() {
    ...
    useEffect(() => {
        ...
        // scene.background = new THREE.Color(0x000000); // Keep it black for the bloom effect to pop, or make it transparent if needed.
        // For Solaris, black background is better for the glow.
        ...
        controls.enableZoom = false; // Disable zoom to prevent interfering with page scroll
        ...
        const bloomPass = new UnrealBloomPass(new THREE.Vector2(width, height), 1.2, 0.8, 0.05);
        composer.addPass(bloomPass);
        const fxaaPass = new ShaderPass(FXAAShader);
        fxaaPass.material.uniforms['resolution'].value.x = 1 / (width * DPR);
        fxaaPass.material.uniforms['resolution'].value.y = 1 / (height * DPR);
        ...
        const handleResize = () => {
            ...
            fxaaPass.material.uniforms['resolution'].value.x = 1 / (newWidth * renderer.getPixelRatio());
            fxaaPass.material.uniforms['resolution'].value.y = 1 / (newHeight * renderer.getPixelRatio());
        };
